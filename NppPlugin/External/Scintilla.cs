...
{
    /// <summary>
    /// Facilitates usage of Scintilla C/C++ functionality.
    /// </summary>
    /// <remarks>'Scintilla' refers to the primary text area seen in Notepad++.</remarks>
    internal abstract class Scintilla
    {
        #region Types
        #region Classes
        public class TextRange : IDisposable
        {
            _TextRange _sciTextRange;
            ...
            {
                if (_ptrSciTextRange != IntPtr.Zero)
                    _sciTextRange = (_TextRange)Marshal.PtrToStructure(_ptrSciTextRange, typeof(_TextRange));
            }
        ...
        internal static event EventHandler Modified = null;
        #endregion
        #region Static Properties
        internal static bool AnnotationsVisible
        {
            set
            {
                IntPtr handle = Notepad.Scintilla;
                if (value)
                {
                    Windows.SendMessage(handle, (uint)Scintilla.Messages.ANNOTATIONSETVISIBLE, 2, 0);
                }
                else
                {
                    Windows.SendMessage(handle, (uint)Scintilla.Messages.ANNOTATIONSETVISIBLE, 0, 0);
                    Windows.SendMessage(handle, (uint)Scintilla.Messages.ANNOTATIONCLEARALL, 0, 0);
                }
                return;
            }
        }
        internal static int CharacterCount
            => Windows.SendMessage(Notepad.Scintilla, (uint)Scintilla.Messages.GETTEXTLENGTH, 0, 0).ToInt32() + 1;
        internal static IntPtr CurrentDocument
            => Windows.SendMessage(Notepad.Scintilla, (uint)Scintilla.Messages.GETDOCPOINTER, 0, 0);
        #endregion
        #region Static Methods
        static Scintilla()
        {
            EntryPoint.Notified += Scintilla.Notified;
            return;
        }
        internal static void AddAnnotation(int line_index, string text)
        {
            IntPtr handle = Notepad.Scintilla;
            Windows.SendMessage(handle, (uint)Scintilla.Messages.ANNOTATIONSETSTYLE, line_index, 1);
            Windows.SendMessage(handle, (uint)Scintilla.Messages.ANNOTATIONSETTEXT, line_index, text);
            return;
        }
        private static void Focus()
        {
            Windows.SendMessage(Notepad.Scintilla, (uint)Scintilla.Messages.SETFOCUS, 1, 0);
            return;
        }
        internal static void GetText(int byte_count, IntPtr pointer)
        {
            Windows.SendMessage(Notepad.Scintilla, (uint)Scintilla.Messages.GETTEXT, byte_count, pointer);
            return;
        }
        internal static void GoToEnd()
        {
            Windows.SendMessage(Notepad.Scintilla, (uint)Scintilla.Messages.SETSEL, -1, -1);
            Scintilla.Focus();
            return;
        }
        internal static void GoToLine(int line)
        {
            // navigate to immediately previous line (guarantees annotation on screen in case where line-in-question is at top of screen)
            Windows.SendMessage(Notepad.Scintilla, (uint)Scintilla.Messages.GOTOLINE, line - 1, 0);
            // navigate to immediate next line (guarantees annotation on screen in case where line-in-question is at bottom of screen)
            Windows.SendMessage(Notepad.Scintilla, (uint)Scintilla.Messages.GOTOLINE, line + 1, 0);
            // navigate to the line
            Windows.SendMessage(Notepad.Scintilla, (uint)Scintilla.Messages.GOTOLINE, line, 0);
            // focus
            Scintilla.Focus();
            return;
        }
        ...
        {
            Windows.SendMessage(Notepad.Scintilla, (uint)Scintilla.Messages.SETMODEVENTMASK,
                (int)(Scintilla.Messages.MOD_INSERTTEXT | Scintilla.Messages.MOD_DELETETEXT), 0);
            Windows.SendMessage(Notepad.Scintilla, (uint)Scintilla.Messages.SETWRAPVISUALFLAGS,
                (int)Scintilla.Messages.WRAPVISUALFLAG_START | (int)Scintilla.Messages.WRAPVISUALFLAG_MARGIN, 0);
        }
        #endregion
